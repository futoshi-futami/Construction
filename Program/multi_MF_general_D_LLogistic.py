# -*- coding: utf-8 -*-
"""
Created on Tue Jan 16 09:56:59 2018
...
def Logistic_regression_Model_prior(X,Y,params,minibatch_U=1):
    ...
    for i in params:
        Minus_Prior +=-i*i       
    return Minus_Log_likelihood,Minus_Prior,model_output
...
def construct_loss(X,Y,dic,Num,Model_prior,D):
    ...
    for  i in range(0,Num*D,D):
        ...
        Minus_Log_likelihood,Minus_Prior,_=Model_prior(X,Y,W)
...
loss,F = construct_loss(X,Y,dictU,Num,Logistic_regression_Model_prior,D)
...
with tf.Session() as sess:
	...
	feed_U=OrderedDict()
	feed_U[X]=Z_0
	feedings4=feed_to_U_MF(feed_U,_Ut,Uniform_t,D)
    
	Z=sess.run(tf.sigmoid(model_output_t), feed_dict=feedings4) 
